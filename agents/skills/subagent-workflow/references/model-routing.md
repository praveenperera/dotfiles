...
Praveen's assessment on 2026-09-04 is that Fable 5.1 is still slightly better at writing code ready for merge and cleaning up code. Treat that as local evidence for when the root may suggest Fable, not a default launch. Fable is opt-in only; see [SKILL.md](../SKILL.md).
...
- Design is understood and the remaining write is ordinary implementation: Luna `max` or Sol; the root may suggest Fable
- Code works but needs focused simplification, removal, or a merge-readiness review: Astra checks the run's complexity-check list unless the user opts into Fable; Sol checks ordinary merge-readiness correctness
- Front-end design: choose Astra or Opus 5; the root may suggest Fable; after the overall design is established, Sol can implement and extend it

Keep the design with the current root when its context is already sufficient. Do not hand off merely to satisfy a model label. Astra can implement a tightly coupled hard fix. Sol can integrate an approved removal and check correctness, but does not decide what to simplify or remove. Do not duplicate investigations or checks that already have sufficient evidence. The table is a preferred division of work, not a capability boundary.
...
**Grok 4.6:** Keep native X research and live evidence with Grok when those tools are useful. Its visual and interactive work can supply a first pass. Sol checks ordinary code quality; Astra checks difficult repository interactions when warranted. The root may suggest Fable. Do not extrapolate a provider's aggregate scores into a universal code-quality ranking. See [Grok 4.6](https://docs.x.ai/developers/grok-4-6).

**Sol:** Praveen's assessment is that Sol is the usual strong, lower-cost root for normal work. Use Sol at `high` for diagnosis, bounded implementation, integration, and acceptance. A Sol run assigned to review is read-only and returns a brief evidence-backed report with checked paths, confirmed defects, and residual risks; this assignment mode is not a Sol-wide prohibition on implementation. In a larger multi-phase goal, a fresh Sol run can review a settled phase's related change set while the Sol root reviews integrated milestones and final acceptance. Do not start one reviewer per package. Independent means a separate run, not a different model. Sol can implement established front-end design and integrate approved removal, but Astra decides simplification and removal unless the user opts into Fable. Escalate to Astra only when evidence leaves a hard or high-risk question unresolved after investigation; a boundary signal alone is not enough. Do not inspect every phase deeply or repeat a routine inventory by default.
