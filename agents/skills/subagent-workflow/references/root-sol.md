...
| Visual or interactive first-pass implementation | Grok 4.6 via Grok CLI (`high` reasoning) |
| Live web or X research that informs a decision | Grok 4.6 via Grok CLI (`high` reasoning) |
| Independent adversarial or inventory-style review | Grok for a third-provider view, a fresh ephemeral `codex exec` Sol pass, or Fable when taste is at stake |
...
- Grok supplies visual or interactive first passes, live web or X research, and a cost-efficient third-provider review.
...
Reach Luna and any Sol subagent with `codex exec`; see [codex-cli.md](codex-cli.md). Reach Grok with the Grok CLI; see [grok-cli.md](grok-cli.md).
...
- Use Grok as an independent provider or for visual and interactive first passes. Keep terminal-heavy debugging and difficult repository surgery in the Sol root unless project evidence supports another route.
