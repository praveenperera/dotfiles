...
4. Preserve the source in `original-spec.md`, then create or revise `spec.md`, `progress.md`, and `audit.md` from [references/templates.md](references/templates.md).
5. Keep `spec.md` a compact controlling router. Move phase detail, context, decisions, audits, and evidence into support files only when needed, and link each from the Read Map.
