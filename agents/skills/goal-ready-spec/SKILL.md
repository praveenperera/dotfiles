...
- Default to `medium` reasoning effort. Use `low` for simple, mechanically verifiable work, and use `high` or `xhigh` only when the task genuinely requires deeper reasoning.
