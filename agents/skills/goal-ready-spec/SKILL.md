...
- Use non-forked subagents selectively during implementation when a bounded, context-isolated task is likely to reduce total token cost. Default to `fork_turns="none"`; use a small fork only when essential recent decisions are absent from durable files, and treat `fork_turns="all"` as exceptional.
...
- For long-running tests, builds, lints, benchmarks, or similar mechanical commands, delegate execution and monitoring to a non-forked low-cost subagent such as `gpt-5.6-luna` at `low` effort when the expected savings exceed delegation overhead. Tell it to use infrequent waits, avoid diagnosing or fixing failures, and report the exact command, exit status, duration, concise result, first actionable failure, and relevant log or artifact paths. The primary agent should not poll the same run concurrently.
...
- Default to `fork_turns="none"` and provide all necessary context in a focused prompt. Use a small fork only when essential recent decisions are not recorded in durable files; treat `fork_turns="all"` as exceptional.
...
- Give each subagent the exact objective, relevant files or commands, constraints, ownership boundary, and expected concise output. Prefer one focused pass and avoid repeated follow-ups or overlapping assignments.
- For a long-running test, build, lint, benchmark, or similar mechanical command, prefer a non-forked low-cost subagent such as `gpt-5.6-luna` at `low` effort when delegation should cost less than having the primary agent monitor it. Give it the exact command and working directory, tell it to wait without frequent polling, and require a structured report containing the command, exit status, duration, concise result, first actionable failure, and relevant log or artifact paths. It must not diagnose failures, edit files, or expand scope unless explicitly assigned. Do not have the primary agent poll the same run concurrently.
