...
- Default implementation subagents to `medium`. When the primary agent decides a higher-level review is warranted, use a fresh-context `high` reviewer for medium-agent work. Use `xhigh` only for genuinely deep ambiguity or high-risk architectural concerns. Do not automatically launch a reviewer after every worker wave; the primary agent remains responsible for integration and decides whether independent review would materially reduce risk.
