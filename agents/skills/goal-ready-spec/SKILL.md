...
- Subagent Delegation: execution-wide rules for non-forked delegation, reasoning-effort selection, bounded prompts and reviews, and primary-agent ownership
...
- Keep an active goal in its current root thread through every phase because a fresh root cannot preserve the goal lifecycle. Never write a fresh-root or fresh-thread transition into a phase boundary, gate, completion criterion, recommended objective, or handoff. Control root context with compact durable checkpoints, progressive disclosure, and compaction; use fresh non-forked subagents for bounded context-isolated work. If the user requests a different root, explain that it requires a separate goal lifecycle and ask how to proceed instead of blocking the active goal.
...
- Every generated `spec.md` must make progress communication event-driven rather than timer-driven. Allow updates for meaningful state changes, actionable failures, integration checkpoints, user decisions, and final results. Prohibit periodic heartbeats whose only content is that work is still running, and require a final result or mailbox report from delegated or background work.
...
- Give each subagent a self-contained prompt with the exact objective, relevant files or commands, constraints, ownership boundary, and expected concise output. Prefer larger, phase-sized, non-overlapping ownership slices over many small assignments, and avoid open-ended investigation, repeated follow-ups, or multiple agents covering the same scope.
- Before launching any delegated review, define its scope, reasoning effort, time or runtime budget, evidence surface, and maximum review rounds. Default review effort to `medium` or `high`; reserve `xhigh` for exceptional architectural, safety-critical, or unusually ambiguous work.
- Default the review sequence to one broad review and at most one targeted follow-up after fixes. Give the follow-up the original findings, implemented fixes, changed hunks, and affected invariants. It must not reread the whole diff unless the fixes materially changed cross-cutting architecture. Never define closure as repeating fresh broad reviews until clean.
- After the review-round cap, the primary agent must continue fixing actionable findings and use integration plus targeted verification to close them. It must record genuine unresolved blockers or residual risk in `progress.md` and `audit.md`; the cap limits review repetition and does not waive completion criteria, evidence requirements, or known blockers.
- Use a fresh non-forked agent for each new worker wave or materially new implementation, proof, or repair slice. Reuse a completed worker only for one brief correction tightly coupled to its immediately completed work when transferring the required context would cost more. Do not interrupt work already in progress.
- When a substantial phase has genuinely independent implementation slices, delegate one or two to fresh workers instead of keeping all implementation in the primary agent while subagents only explore, review, or check. Keep architecture, integration, and cross-cutting fixes with the primary agent. Do not manufacture parallel work when slices are tightly coupled or delegation overhead would exceed the savings.
- Default implementation work to at most two concurrent workers. Require each worker to verify its own slice and return one concise final report with changed files, verification results, risks, and integration notes. While a worker wave is active, the primary agent may do unrelated work but should not reread worker-owned files or duplicate focused verification. After the wave finishes, require one integration checkpoint to inspect the combined diff, reconcile boundaries, run cross-cutting checks, and update durable progress or audit state before launching another wave. Exceed two concurrent implementation workers only when additional slices are genuinely independent and the expected savings clearly exceed coordination and context costs.
- Delegate long-running mechanical checks to a non-forked low-cost subagent such as `gpt-5.6-luna` at `low` effort when that should save tokens. Give it the exact command and working directory, and require exactly one final response containing the command, exit status, duration, concise result, first actionable failure, and artifact paths. It must not send separate or interim mailbox messages, diagnose, edit, or expand scope unless assigned; the primary agent may do unrelated work but must not duplicate the run.
- Require generated specs to choose each wait timeout for a long-running command or subagent from the expected remaining duration, capped at five minutes, and to prefer one appropriately sized wait over repeated short polls.
- Keep contract interpretation, cross-cutting architectural decisions, integration, and final responsibility with the primary agent. The primary agent must inspect and verify delegated results before relying on them.
- If the user explicitly asks for independent or subagent completion review, define it as a final-only read-only completion gate, not a routine phase or progress-review step, unless the user explicitly requests earlier review. It should begin only after the implementing agent believes all criteria are satisfied and `audit.md` has been filled, and it remains subject to the bounded review sequence.
- A completion-review subagent, when requested and available, should compare `spec.md`, `original-spec.md`, `progress.md`, `audit.md`, supporting files referenced by uncertain audit items, and the current uncommitted diff. It must search for counter-evidence and classify Hard Requirements, Architecture and Ownership Invariants, No Equivalent Substitutions, deviations, residual risk, verification scope, and acceptance criteria. If its findings lead to fixes, any follow-up is the targeted final round described above rather than a fresh whole-diff review.
...
- Do not interrupt active implementation or verification solely to adopt this review policy. Apply it at the next review decision and record the chosen review contract before launching another reviewer.
...
- Default to `fork_turns="none"` and provide all necessary context in a focused prompt. Use the smallest positive fork only when essential recent decisions are not recorded in durable files. Do not use `fork_turns="all"` unless the user explicitly requests it.
- Default implementation subagents to `medium`. Default reviewers to `medium` or `high`; use `xhigh` only for exceptional architectural, safety-critical, or unusually ambiguous work. Do not automatically launch a reviewer after every worker wave; the primary agent remains responsible for integration and decides whether independent review would materially reduce risk.
- Give each subagent the exact objective, relevant files or commands, constraints, ownership boundary, and expected concise output. Prefer larger, phase-sized, non-overlapping ownership slices over many small assignments, and avoid repeated follow-ups or overlapping work.
- Before each delegated review, record its explicit scope, reasoning effort, time or runtime budget, evidence surface, and maximum review rounds.
- Default to one broad review and at most one targeted follow-up after fixes. Give the follow-up the original findings, implemented fixes, changed hunks, and affected invariants; it must not reread the whole diff unless the fixes materially changed cross-cutting architecture. Never define completion as repeating fresh broad reviews until clean.
- After the review-round cap, continue fixing actionable findings through primary-agent integration and targeted verification. Record any genuine unresolved blocker or residual risk in the progress and audit files. The cap limits review repetition; it does not waive completion criteria, evidence requirements, or known blockers.
- Use a fresh non-forked agent for each new worker wave or materially new implementation, proof, or repair slice. Reuse a completed worker only for one brief correction tightly coupled to its immediately completed work when transferring the required context would cost more. Do not interrupt work already in progress.
- When a substantial phase has genuinely independent implementation slices, delegate one or two to fresh workers instead of keeping all implementation in the primary agent while subagents only explore, review, or check. Keep architecture, integration, and cross-cutting fixes with the primary agent. Do not manufacture parallel work when slices are tightly coupled or delegation overhead would exceed the savings.
- Default implementation work to at most two concurrent workers. Each worker must verify its own slice and return one concise final report with changed files, verification results, risks, and integration notes. While a worker wave is active, the primary agent may do unrelated work but should not reread worker-owned files or duplicate focused verification. After all workers in the wave finish, perform one integration checkpoint: inspect the combined diff, reconcile boundaries, run cross-cutting checks, and update durable progress or audit state before launching another wave. Exceed two concurrent implementation workers only when additional slices are genuinely independent and the expected savings clearly exceed coordination and context costs.
- For a long-running mechanical check, prefer a non-forked low-cost subagent such as `gpt-5.6-luna` at `low` effort when that should save tokens. Give it the exact command and working directory, and require exactly one final response containing the command, exit status, duration, concise result, first actionable failure, and artifact paths. It must not send separate or interim mailbox messages, diagnose, edit, or expand scope unless assigned; the primary agent may do unrelated work but must not duplicate the run.
- When waiting for a long-running command or subagent, choose the wait timeout from the expected remaining duration, capped at five minutes. Prefer one appropriately sized wait over repeated short polls.
- Keep contract interpretation, cross-cutting decisions, integration, and final verification with the primary agent. Inspect and verify delegated results before relying on them.
```
...
- Completion criteria:
- Final completion review contract and rounds:
...
If the goal continues across turns or resumes after compaction, the agent should use progressive disclosure: read the router spec's hot sections, `progress.md`, current repo state, and only the supporting files selected by the Read Map for the next action or completion decision. The active goal stays in its current root thread through every phase; it must not plan or require a fresh-root transition. Use compact durable state, the Read Map, progressive disclosure, and compaction to control root context, and use fresh non-forked subagents for bounded context-isolated work. When finishing a phase, it should summarize the phase outcome in `progress.md` before moving on. It should compress hot files only when they exceed target size, duplicate old evidence, reach a phase handoff, prepare for final audit, or must survive likely compaction. After failed or partial verification, it should inspect the evidence, make the smallest contract-preserving next change, rerun the relevant focused verification, and update progress. If budget is exhausted or the next step requires missing input, unavailable resources, manual intervention, or a contract change, it should continue any other agent-actionable work that can still be done within the contract. Only when every remaining unsatisfied item is an irreducible blocker should it record the terminal blocker, report completed work, unsatisfied criteria, verification state, attempts, and the needed decision, then mark the goal complete so it does not loop.

On the final completion pass only, before calling `update_goal`, the agent should load `audit.md`, the relevant original-spec source or pointer, Completion Criteria, Acceptance Criteria, Verification Plan, and only the phase audits or supporting files referenced by missing or uncertain audit items. It should confirm the final implementation and completed spec covered every binding material requirement from the original spec source, then run the required counter-evidence scan against the current diff and relevant files. If the user or spec required independent completion review, begin the bounded review sequence only at this final point after `audit.md` is filled unless the user explicitly requested earlier review; if the review is unavailable, perform the specified read-only fallback audit and record it in `audit.md`. After fixes, any permitted follow-up must receive the original findings, fixes, changed hunks, and affected invariants and remain targeted unless the fixes materially changed cross-cutting architecture. Once the review-round cap is reached, the primary agent owns further fixes and targeted verification without waiving unresolved blockers or completion evidence. It should not repeat the original-spec coverage pass, counter-evidence scan, or broad completion review during routine continuation, progress updates, or phase handoffs.
