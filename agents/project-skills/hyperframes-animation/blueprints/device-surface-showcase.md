...
- floating-window establish + the surface presented as a tilted/floating UI card → [3d-page-scroll](../rules/3d-page-scroll.md) for tilt and perspective framing plus [CSS 3D transforms](../techniques.md#3-css-3d-transforms) for `translateZ` depth
...
- 3D device drift + self-rotate-to-camera + perspective depth (3d-hand) → [CSS 3D transforms](../techniques.md#3-css-3d-transforms) or the [Three.js adapter](../adapters/three.md); see camera modifier
...
- 3D-hand entrance + swipe + tap as the interaction DRIVER (gesture input that scrolls/selects) → **flagged special — needs a heavier capability beyond the rule library (R3F/Three.js + WebGL), NOT a motion-shape rule.** Use the [Three.js adapter](../adapters/three.md) for the 3D hand and WebGL bloom, but author the swipe-to-scroll and tap-to-select gesture protocol explicitly; `context-sensitive-cursor` and `camera-cursor-tracking` model only a flat cursor.
- zoom-through bloom / portal exit (3d-hand) → **flagged special — needs a heavier capability beyond the rule library (WebGL), NOT a named transition rule.** Use the [WebGL shader technique](../techniques.md#13-webgl-fragment-shader-art), but author the bloom or portal fly-through because no named transition rule covers it.
...
- 3d-hand → ONE continuous forward push (no cuts) → [multi-phase-camera](../rules/multi-phase-camera.md) in steady-push mode (1.0→1.03→1.06… plus its sine micro-drift), layered over [CSS 3D transforms](../techniques.md#3-css-3d-transforms) or the [Three.js adapter](../adapters/three.md) so the device self-rotates to the lens; the push runs unbroken into the bloom or portal exit.
