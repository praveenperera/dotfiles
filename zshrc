# fall back to xterm-256color if current TERM has no terminfo entry
...
sf() { cmd tmux sync-ssh --all && unset SSH_CONNECTION SSH_CLIENT; }

# jj
alias jjt="ju tree"
alias jjtf="jju tree -f"
alias jjs="jju ss"
alias jjc="jju clean"
...
alias clo="claude --permission-mode plan --dangerously-skip-permissions --model opus --effort medium"

clx() {
  if [[ -z ${CLIPROXY_API_KEY:-} ]]; then
    print -u2 "clx: CLIPROXY_API_KEY is not set in ~/.secrets.zsh"
    return 1
  fi
  ...
  ANTHROPIC_BASE_URL="http://127.0.0.1:8317" \
    ...
    ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION="GPT-5.6 Sol via local CLIProxyAPI" \
...
alias cls="claude --permission-mode acceptEdits --model sonnet"
alias clh="claude --permission-mode acceptEdits --model haiku"
...
alias luh="cmd codex usage-history"
alias lud="luh -d 5"
...
# grok
alias gg="grok -m grok-4.7 --effort high"
...
export PATH=/Users/praveen/.opencode/bin:$PATH
...
# shift-tab to accept autosuggestion (non-tmux only)
if [[ -z "$TMUX" ]]; then
