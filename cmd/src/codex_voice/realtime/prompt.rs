...
Start the readout immediately with the Codex message content. Do not introduce it, summarize it first, or say phrases like "here is the message" or "read back faithfully".
...
pub fn readout_message(item: &ReadableItem) -> String {
    format!(
        "Task: read the Codex {kind:?} content below aloud, starting with the first word of the content. After the readout, discuss follow-up questions. When I say I am ready, produce FINAL_PROMPT.\n\nCodex content:\n{message}",
        kind = item.kind,
        message = item.text
    )
}
